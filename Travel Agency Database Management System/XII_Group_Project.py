# disabling vscode extension for spell check
# /* cSpell:disable */
...
print(
    """Welcome to Manage-My-Yatra, A simple console based program
...
def book():
    ...
    ask2 = input(
        "\nYou are going to book a ticket to "
        + list2[2]
        + ", "
        + list2[3]
...
def add_destination():
    ...
    while x < 1:
        try:
            l1 = eval(
                input(
                    "Enter details of new destination as a list ['DestinationName', 'Location', Cost]:"
                )
            )
            l2 = eval(
                input(
                    "Enter travel costs of new destination as a list [Roadways , Railways , Waterways , Airways]:"
                )
            )
            x = 2
        except Exception as error:
...
def edittravelcost():
    ...
    mode1 = input(
        "Enter the mode of transport whose value is to be edited(Roadways,Railways,Waterways,Airways):"
    )
    new_cost = input("Enter the new cost:")
    query2 = (
        """UPDATE travel_cost
